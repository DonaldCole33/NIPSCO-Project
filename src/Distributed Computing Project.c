...
void ComputeCorrelation(struct Correlation* cData){
    ...
    for (int i = startMonth; i < endMonth; i++){	//Compute CF for each month
        for (int j = 0; j < 31; j++){
            if (cData->outtageDataX[i][j] > 0 && cData->outtageDataX[i][j] < 50){
                currentSampleSize++;
...
void cb1 (void *src, size_t len, void *cData) {
    ...
    char* cellData;
    struct tm *datetofind;

    if (len != 0){
        csv_write(buf, 100, src, len);
    ...
    while(datetofind == NULL){
        datetofind = &mycData->dateArray[--sampleSize - 1];
        printf("NULL DATE\n");
    }
    while((datetofind->tm_mon < 0 || datetofind->tm_mon > 11)
          || (datetofind->tm_mday < 0 || datetofind->tm_mday > 31)
          || (datetofind->tm_hour < 0 || datetofind->tm_hour > 23)){
        printf("DATE ERROR\n");
        //*mycData->dateArray[sampleSize -1] = NULL;               //delete date to use
        datetofind = &mycData->dateArray[--sampleSize -1];
    }
    ...
    if (monthFlag && dayFlag && hourFlag && CURRENT_COLUMN_NUM == weatherDataColumn){	//found data
        if (strcmp(cellData, "") == 0){
            ...
            mycData->outtageDataX[datetofind->tm_mon][datetofind->tm_mday - 1]--;		//Remove the outage data
            ...
            printf("Mon to find = %d, Day to find = %d\n",datetofind->tm_mon, datetofind->tm_mday-1 );
...
void cb3 (void *src, size_t len, void *cData) {
    ...
    if (len != 0){
        csv_write(buf, 50, src, len);						//Copy to Buffer for easy parsing & string Management
    ...
    if (CURRENT_OUTTAGE_ROW_NUM > 0){						//to access the correct Data and not the header
        if (CURRENT_COLUMN_NUM == currentLOAColumn){		//Checking the LOA Column
            if(mycData->LOA == atoi(myCellData)){			//Check for the correct LOA
                ...
                validRowFlag = CURRENT_OUTTAGE_ROW_NUM;		//We are on the correct LOA Row
            }
        }
        ...
        //we can get the matching weather data and Cause
            ...
            memset(&date, INT_MAX, sizeof (date));\
            if(!strptime(myCellData, "%Y/%m/%d %H:%M:%S", &date)){
                validRowFlag = 0;
                if (sampleSize == 42){
                    printf("MONTH=%d\n",date.tm_mon);
                }
    ...
    if (validDateFlag &&
        ...
        if(date.tm_mon >= myMonth &&
           date.tm_mon < (myMonth + durationofMonths) ){
            ...
            if (CheckDate(&date)){							//Do some work and add the date to get the weather data
                ...
                memcpy(&mycData->dateArray[sampleSize++], &date, sizeof(struct tm));	//This is the date we need to match in the weather data
                printf("%d LOA=%d, Row=%d, DATE=%d/%d/%d\n",
                       sampleSize,
                       currentLOA,
                       CURRENT_OUTTAGE_ROW_NUM,
                       mycData->dateArray[sampleSize - 1].tm_mon,
                       mycData->dateArray[sampleSize-1].tm_mday,
                       mycData->dateArray[sampleSize-1].tm_year);
        ...
        else{	//Reset flags because the time is not in the same area
            validDateFlag = 0;					//reset flag
            validRowFlag = 0;					//Reset Flag
            validCauseFlag = 0;
        }
...
int main(int argc, char **argv){
    ...
    cData->dateArray = malloc(1000 * sizeof(struct tm*));
//    for (int i = 0; i < 1000; i++){
//        cData->dateArray[i] = malloc(sizeof(struct tm*));
//    }

    cData->weatherDataY = malloc(r * sizeof(double *));
    cData->outtageDataX = malloc(r * sizeof(int*));

    for (int i = 0; i < r; i++){
        cData->weatherDataY[i] = malloc(c * sizeof(double*));
        cData->outtageDataX[i] = malloc(c * sizeof(int*));
    }
